...
def modify():
    if request.form["modify"] == "edit":
        ...
        pkmn = read_pkmn_by_pkmn_id(pkmn_id)
        delete_pet(pkmn_id)
        return redirect(url_for("pkdex", pkmn_type=pkmn["pokemon_type"]))

@app.route('/update', methods=['POST'])
def update():
    pkmn_data = {
        "pokemon_id" : request.form["pkmn_id"],
        "pokemon_type": request.form['pkmn_type'],
        "pokemon_name": request.form['pkmn_name'],
        "pokemon_description": request.form['pkmn_desc'],
        "pokemon_url": request.form['pkmn_url']
    }
    update_pkmn(pkmn_data)
    return redirect(url_for('pet', pkmn_id=request.form['pkmn_id']))